...
function doInsert(){
	...
	function doEdit(){
  	...
  	// 		$res = $customer->single_customer($_GET['customerid']); 
	        ...
	        $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$cur->PHONE."','Janno','FROM Bachelor of Technology : Your order has been '".$status. "'. The amount is '".$cur->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
			...
			foreach ($cur as $result) {
	  		 ...
	  		 $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$result->OWNERPHONE."','Janno','FROM Bachelor of Technology: Your  product has been ordered'. The amount is '".$result->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
